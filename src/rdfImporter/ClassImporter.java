...
public class ClassImporter {
    ...
    public static boolean loadClassIn(OntClass ontClass) throws Exception{
    ...
    public static boolean loadClassRelIn(OntClass ontClass1, OntClass ontClass2, CLASS_REL rel) throws Exception {
        String fPre = CypherUtil.getPreLabel(ontClass1.getURI());
        String lPre = CypherUtil.getPreLabel(ontClass2.getURI());
        if(!CacheClass.isRelExisted(fPre,lPre)){
            String cypher = CypherUtil.intoRelCypher(ontClass1,ontClass2,rel);
            Neo4jConnection.getSession().writeTransaction(new TransactionWork<Integer>() {
                @Override
                public Integer execute(Transaction transaction) {
                    StatementResult mRst = transaction.run(cypher);
